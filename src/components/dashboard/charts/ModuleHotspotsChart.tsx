  ...
  if (isLoading) {
    return (
      <Card className="border-border/50 bg-card shadow-card">
        <CardHeader>
          <CardTitle className="text-lg font-bold">Course Leaderboard</CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Top courses by absence rate (min 200 records)
            </p>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        {topHotspots.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-[200px] text-muted-foreground">
            ...
            <p className="text-xs">Adjust filters or check minimum sample size</p>
          </div>
        ) : (
            ...
            <Table>
              ...
              <TableBody>
                {topHotspots.map((hotspot, idx) => (
                  <TableRow key={`${hotspot.course_code}-${idx}`} className="text-sm">
                    ...
                    <TableCell className="text-xs text-muted-foreground">
                      {hotspot.programme_level}
                    </TableCell>
                    <TableCell className="text-center">
                      <Badge 
                        variant="secondary" 
                        className={`text-xs ${getAttendanceBg(hotspot.attendance_rate)}`}
                      >
                        {hotspot.attendance_rate}%
                      </Badge>
                    </TableCell>
                    <TableCell className="text-center">
                      <Badge 
                    ...
                    <TableCell className="text-right text-xs text-muted-foreground">
                      {hotspot.total_records.toLocaleString()}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
